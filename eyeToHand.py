...
if __name__ == '__main__':
    ...
    file_path = "./collect_data/poses.txt" #采集标定板图片时对应的机械臂末端的位姿 从 第一行到最后一行 需要和采集的标定板的图片顺序进行对应
    ...
    rotation_matrix ,translation_vector, camera_mtx, camera_dist = compute_T(images_path,corner_point_long,corner_point_short,corner_point_size)
    print('内参矩阵')
    print(camera_mtx)
    print('畸变系数')
    print(camera_dist)
    ...
    np.savetxt('./measured_data/mtx.txt', camera_mtx)
    np.savetxt('./measured_data/dist.txt', camera_dist)
    np.savetxt('./measured_data/rotation_matrix.txt', rotation_matrix)
    np.savetxt('./measured_data/translation_vector.txt', translation_vector)
    np.savetxt('./measured_data/camera_pose.txt', homogeneous_matrix)
